...
#static var _current_log_file_path: String
...
func _init(log_name: String = "Main", min_log_level := LogLevel.DEFAULT, crash_behavior: Callable = default_crash_behavior):
	_log_name = log_name
...
func err_cond_not_ok(err_code: Error, message: String, _fatal := true, other_values_to_be_printed = {}):
	if err_code != OK:
		call_thread_safe("_internal_log", message + "" if message.ends_with(".") else "." + " Error string: " + error_string(err_code), other_values_to_be_printed, LogLevel.FATAL if _fatal else LogLevel.ERROR)

##Throws an error if the "statement" passed is false. Handy for making code "free" from if statements.
func err_cond_false(statement: bool, message: String, _fatal := true, other_values_to_be_printed = {}):
	if !statement:
		call_thread_safe("_internal_log", message, other_values_to_be_printed, LogLevel.FATAL if _fatal else LogLevel.ERROR)

##Throws an error if argument == null
func err_cond_null(arg, message: String, _fatal := true, other_values_to_be_printed = {}):
	if arg == null:
		call_thread_safe("_internal_log", message, other_values_to_be_printed, LogLevel.FATAL if _fatal else LogLevel.ERROR)

##Throws an error if the arg1 isn't equal to arg2. Handy for making code "free" from if statements.
func err_cond_not_equal(arg1, arg2, message: String, _fatal := true, other_values_to_be_printed = {}):
	...
	if (arg1 is Color && arg2 is Color && !arg1.is_equal_approx(arg2)) || arg1 != arg2:
		call_thread_safe("_internal_log", str(arg1) + " != " + str(arg2) + ", not allowed. " + message, other_values_to_be_printed, LogLevel.FATAL if _fatal else LogLevel.ERROR)
...
func _log_mode_editor(msg: String, lvl: LogLevel, stack: Array):
	match lvl:
		LogLevel.DEBUG:
			...
			print_rich("[color=orange]" + msg + "[/color]")
		LogLevel.INFO:
			msg = msg.replace("[lb]", "[").replace("[rb]", "]")
			print_rich("[color=green]" + msg + "[/color]")
		LogLevel.WARN:
			msg = msg.replace("[lb]", "[").replace("[rb]", "]")
