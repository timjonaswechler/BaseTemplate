...
var settingsFilePath: String = OS.get_user_data_dir() + "/settings.%s" % FileFormat
...
func _ready() -> void:
	if load_on_start:
		Log.info("SettingsHandler started")
		prepare_settings()

# wie verbinde ich das Singal closing von WindwoManager
func _notification(what: int) -> void:
	if what == NOTIFICATION_EXIT_TREE:
		Log.info("Settings will be saved")
		save_settings()
...
#region Generic
func save_settings(path: String = settingsFilePath) -> void:
...
func reset_to_factory_settings(path: String = settingsFilePath) -> void:
...
func load_settings(path: String = settingsFilePath) -> void:
	...
	var required_sections = [
		GameSettings.VersionSection,
		GameSettings.AudioSection,
		GameSettings.GraphicsSection,
		# GameSettings.ControlsSection,
		GameSettings.KeybindingsSection,
		GameSettings.LocalizationSection,
		GameSettings.AnalyticsSection,
		GameSettings.AccessibilitySection
	]

	for required_section in required_sections:
		if not required_section in config_file_api.get_sections():
			match required_section:
				GameSettings.VersionSection:
...
func remove_settings_file(path: String = settingsFilePath) -> void:
...
#region Creation
func create_settings(path: String = settingsFilePath) -> void:
	create_version_section()
	create_audio_section()
	create_graphics_section()
	create_accessibility_section()
...
func create_version_section() -> void:
	update_version_section(GameSettings.MajorVersionSetting, Engine.get_version_info().major)
	update_version_section(GameSettings.MinorVersionSetting, Engine.get_version_info().minor)
	update_version_section(GameSettings.PatchVersionSetting, Engine.get_version_info().patch)
	update_version_section(GameSettings.StatusVersionSetting, Engine.get_version_info().status)
	update_version_section(GameSettings.BuildVersionSetting, Engine.get_version_info().build)

func create_audio_section() -> void:
	update_audio_section(GameSettings.CurrentAudioOutputDeviceSetting, GameSettings.DefaultSettings[GameSettings.CurrentAudioOutputDeviceSetting])
	...
	if (GameSettings.DefaultSettings[GameSettings.MutedAudioSetting]):
...
func load_audio() -> void:
	...
	var currentOutputDevice: String = get_audio_section(GameSettings.CurrentAudioOutputDeviceSetting)
	AudioManager.set_audio_device(currentOutputDevice)
...
func get_version_section(key: String):
	return config_file_api.get_value(GameSettings.VersionSection, key)
...
func update_version_section(key: String, value: Variant) -> void:
	config_file_api.set_value(GameSettings.VersionSection, key, value)
	updated_setting_section.emit(GameSettings.VersionSection, key, value)
...
#region Section Checkers
func check_version_section():
	pass


func check_audio_section() -> void:
	if not GameSettings.CurrentAudioOutputDeviceSetting in config_file_api.get_section_keys(GameSettings.AudioSection):
		var currentOutputDevice: String = GameSettings.DefaultSettings[GameSettings.CurrentAudioOutputDeviceSetting]
		update_audio_section(GameSettings.CurrentAudioOutputDeviceSetting, currentOutputDevice)
		
	if not GameSettings.MutedAudioSetting in config_file_api.get_section_keys(GameSettings.AudioSection):
	...
	for bus in AudioManager.available_buses:
		if not bus in config_file_api.get_section_keys(GameSettings.AudioSection):
			update_audio_section(bus, AudioManager.get_default_volume_for_bus(bus))
		
func check_localization_section() -> void:
	if not GameSettings.CurrentLanguageSetting in config_file_api.get_section_keys(GameSettings.LocalizationSection):
		update_localization_section(GameSettings.CurrentLanguageSetting, GameSettings.DefaultSettings[GameSettings.CurrentLanguageSetting])
	
	if not GameSettings.VoicesLanguageSetting in config_file_api.get_section_keys(GameSettings.LocalizationSection):
		update_localization_section(GameSettings.VoicesLanguageSetting, GameSettings.DefaultSettings[GameSettings.VoicesLanguageSetting])
	
	if not GameSettings.SubtitlesLanguageSetting in config_file_api.get_section_keys(GameSettings.LocalizationSection):
		update_localization_section(GameSettings.SubtitlesLanguageSetting, GameSettings.DefaultSettings[GameSettings.SubtitlesLanguageSetting])
	
	if not GameSettings.SubtitlesEnabledSetting in config_file_api.get_section_keys(GameSettings.LocalizationSection):
		update_localization_section(GameSettings.SubtitlesEnabledSetting, GameSettings.DefaultSettings[GameSettings.SubtitlesEnabledSetting])
